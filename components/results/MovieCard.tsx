...
// Gradient par genre pour le fallback d'affiche
...
// Style des badges de plateforme
...
export default function MovieCard({ scoredMovie, rank }: MovieCardProps) {
  ...
  return (
    <motion.article
      className="group flex flex-col rounded-2xl overflow-hidden bg-zinc-900/80 border border-white/[0.07] hover:border-white/[0.14] transition-colors duration-300"
      ...
      <div className="relative aspect-[2/3] w-full overflow-hidden bg-zinc-900 shrink-0">
        {movie.posterUrl ? (
          ...
          <div className={`absolute inset-0 bg-gradient-to-b ${gradient} flex flex-col items-center justify-center`}>
            <span className="text-6xl sm:text-7xl font-black text-white/10 select-none leading-none">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-zinc-900 via-zinc-900/30 to-transparent opacity-80" />

        {/* Badge rang */}
        <div className="absolute top-3 left-3 w-8 h-8 rounded-xl bg-amber-500 text-zinc-950 text-xs font-black flex items-center justify-center shadow-xl shadow-amber-500/30">
          {rank}
        </div>

        {/* Badges plateforme — glass */}
        ...
        {/* Raison de recommandation en bas de l'affiche */}
